...
module.exports = function(app) {
  ...
  app.get("/users/login",function(req, res) {
      res.render("login", {log_flag:true});
  });
  ...
  passport.use(new passportlocal.Strategy(function(username, password, done) {
    //locating the username within our db.
    ...
    db.Users.findOne({
      ...
      bcrypt.compare(password, data.password, function(err, isMatch) {
        ...
        console.log(isMatch);
        if(isMatch) {
            done(null, data, {message: "You are logged in!" });
        }
        else {
  ...
  app.post("/users/login", 
    passport.authenticate('local', { successRedirect: '/users/wallet',
                                    ...
                                    failureFlash: true }),
    function(req, res){
        console.log("you have logged in man!!!");
    });
  ...
  // I added these routes..
    ...
    app.post('/users/wallet', function(req, res){
        // const created_at = new Date();
        var total;
        const newSpending = req.body.comment;
        db.Spendings.create({
            GroupId: req.session.passport.user,
            groceries: req.body.groceries,
            gas: req.body.groceries,
